...
echo "Testing third increment"
for cas_de_test in src/test/deca/context/valid/increment3/*.deca
do
    nb_tests=$((nb_tests+1))
    test_context_incr3_valid "$cas_de_test" 2>/dev/null
done
    for cas_de_test in src/test/deca/context/invalid/increment3/*.deca
    do
        nb_tests=$((nb_tests+1))
        test_context_incr3_invalid "$cas_de_test" 2>/dev/null
    done
        if [ $passed -eq 1 ] #j'ai remplacé = par -eq et j'ai rajouté les crochets
        then 
            echo ${e} "Third increment : ${Green}PASSED${NC}"
        else
            echo 
            echo ${e} "Third increment : ${Red}FAILED${NC}"
