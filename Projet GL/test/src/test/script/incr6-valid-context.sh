...
echo "Testing valid tests sixth increment"
for cas_de_test in src/test/deca/context/valid/increment6/*.deca
do
    nb_tests=$((nb_tests+1))
    test_context_incr6_valid "$cas_de_test" 2>/dev/null
done
        if [ $passed -eq 1 ] #j'ai remplacé = par -eq et j'ai rajouté les crochets
        then 
            echo ${e} "Valid tests sixth increment : ${Green}PASSED${NC}"
        else
            echo 
