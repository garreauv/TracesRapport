...
echo "Testing valid tests first increment"
for cas_de_test in src/test/deca/context/valid/increment4/*.deca
do
    nb_tests=$((nb_tests+1))
    test_context_incr4_valid "$cas_de_test" 2>/dev/null
done
        if [ $passed -eq 1 ] #j'ai remplacé = par -eq et j'ai rajouté les crochets
        then 
            echo ${e} "Valid tests fourth increment : ${Green}PASSED${NC}"
        else
            echo 
