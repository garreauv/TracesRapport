...
echo "Testing first increment"
for cas_de_test in src/test/deca/context/valid/increment1/*.deca
do
    nb_tests=$((nb_tests+1))
    test_context_incr1_valid "$cas_de_test" 2>/dev/null
done
    for cas_de_test in src/test/deca/context/invalid/increment1/*.deca
    do
        nb_tests=$((nb_tests+1))
        test_context_incr1_invalid "$cas_de_test" 2>/dev/null
    done
        if [ $passed -eq 1 ] #j'ai remplacé = par -eq et j'ai rajouté les crochets
        then 
            echo ${e} "First increment : ${Green}PASSED${NC}"
        else
            echo 
            echo ${e} "First increment : ${Red}FAILED${NC}"
